...
const FETCH_STUDENTS = route(`teacher.academic.record.classes.fetch`);
...
async function fetchClasses(search = '', page = 1, pageSize = 10, year_level = '', section = '', semester  = '', school_year = '') {
    try {
        const response = await fetch(`${FETCH_STUDENTS}?search=${search}&page=${page}&pageSize=${pageSize}
            &year_level=${year_level}&section=${section}&semester=${semester}&school_year=${school_year}`);
    ...
    manageClasses.forEach((manageClass, index) => {
        ...
        const row = `
            <tr>
                ...
                <td>
                    <div class="actions">
                        <a href="${route(`teacher.view.academic.records`, { class_record: (manageClass.class_management_id) }) }" class="view" title="VIEW">
