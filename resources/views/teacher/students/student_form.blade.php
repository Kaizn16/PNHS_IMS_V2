    ...
    <section class="wrapper">
        ...
        <div class="content-box">
            <div class="form-container">
                <form action="{{ isset($student) ? route('teacher.update.student', ['student_id' => $student->student_id]) : route('teacher.store.student') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-actions">
                        ...
                        <a href="{{ route('teacher.students') }}"><i class="material-icons icon">cancel</i>CANCEL</a>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    ...
    let selectedAdviser = @json(isset($teacher) ? $teacher->teacher_id : old('teacher'));
    let existingAdviser = @json($teacher ?? null);
    
    if (existingAdviser) {
        let option = new Option(
            `${existingAdviser.first_name} ${existingAdviser.middle_name?.[0] ? existingAdviser.middle_name[0] + "." : ''} ${existingAdviser.last_name}`,
            existingAdviser.teacher_id,
            true,
            true
        );
        $('#teacher').append(option).trigger('change'); // Add and select option
    }
