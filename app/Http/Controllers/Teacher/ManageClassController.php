...
class ManageClassController extends Controller
{
    ...
    {   
        ...
        return view('teacher.manage_class.manage_class', compact('schoolYears'));
    }
    ...
    public function fetchClasses(Request $request)
    {
        ...
        $is_deleted = filter_var($request->input('is_deleted', false), FILTER_VALIDATE_BOOLEAN);
    ...
    public function create()
    {
        ...
        return view('teacher.manage_class.manage_class_form', compact('teacher'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->day as $index => $day) {
                ...
                if ($conflict) {
                    DB::rollBack();
                    return redirect()->route('teacher.create.class')->withInput()->with([
                        'type' => 'error',
            ...
            return redirect()->route('teacher.manageclass')->with([
                'type' => 'success',
    ...
    public function edit(string $class_management_id)
    {
        ...
        if($class) {
            ...
            return view('teacher.manage_class.manage_class_form', compact('class', 'teacher', 'students', 'schedules'));
        }
    ...
    public function update(Request $request, string $class_management_id)
    {
        ...
        try {
            $class = ClassManagement::findOrFail($class_management_id);
            ...
            foreach ($request->day as $index => $day) {
                ...
                if ($conflict) {
                    DB::rollBack();
                    return redirect()->route('teacher.edit.class', $class_management_id)->withInput()->with([
                        'type' => 'error',
                        'message' => 'Schedule conflict detected on ' . $day . ' between ' . $request->start_time[$index] . ' and ' . $request->end_time[$index]
                    ]);
                }
    ...
    public function view(string $class_management_id) 
    {
        ...
        if($manageClass) {
            ...
            $schedules = Schedule::where('class_management_id', $class_management_id)->get();
            return view('teacher.manage_class.view_class', compact('manageClass', 'totalStudents', 'schedules'));
        }
